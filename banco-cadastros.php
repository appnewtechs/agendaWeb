...
// 	$offset = ($page - 1) * $items_per_page;
// 	$clientes = array();
// 	$query = "select * from cliente limit {$offset},{$items_per_page}";
// 	$query = "select cliente.*,trabalho.descricao as nome_trabalho,tipo_cliente.descricao as nome_tipo_cliente FROM `cliente` INNER JOIN `trabalho` ON cliente.id_trabalho = trabalho.id_trabalho INNER JOIN `tipo_cliente` ON cliente.id_tipo_cliente = tipo_cliente.id_tipo_cliente LIMIT {$offset},{$items_per_page}";
// 	// 
// 	$resultado = mysqli_query($conexao, $query);
// 	while ($cliente = mysqli_fetch_assoc($resultado)) {
// 		$query_contato = "select * from contato WHERE id_cliente = {$cliente['id_cliente']}";
// 		$cliente['contatos'] = "";
// 		$cliente['emails_contato'] = "";
// 		$resultado_contato = mysqli_query($conexao, $query_contato);
// 		$numResults = mysqli_num_rows($resultado_contato);
// 		$counter = 0;
// 		while ($contato = mysqli_fetch_assoc($resultado_contato)) {
// 			if (++$counter == $numResults) {
// 		        // last row
// 		    	$cliente['contatos'] .= $contato['nome'];
// 		    	$cliente['emails_contato'] .= $contato['email'];	
// 		    } else {
// 		        // not last row
// 				$cliente['contatos'] .= $contato['nome']."<br>";
// 				$cliente['emails_contato'] .= $contato['email']."<br>"; 
// 		    }
// 		}
...
// function buscaLinhasProdutos($conexao){
// 	$query = "select * from linha_produto";
// 	$resultado = mysqli_query($conexao, $query);
// 	return $resultado;
// }
...
// function buscaNomeFantasiaRazaoSocial($conexao,$cnpj,$nome_fantasia,$razao_social,$id_cliente){
// 	$query = "select * from cliente where id_cliente = {$id_cliente}";
// 	$resultado = mysqli_query($conexao, $query);
// 	$ok = 0;
// 	while ($cliente = mysqli_fetch_assoc($resultado)) {
// 		if($cliente['nome_fantasia'] != $nome_fantasia){$ok = $ok+1;}
// 		if($cliente['razao_social'] != $razao_social){$ok = $ok+1;}
// 		if($cliente['cnpj'] != $cnpj){$ok = $ok+1;}
// 	}
// 	var_dump($ok);
// 	if($ok == 4 || $ok == 0){
// 		$query = "select * from cliente where nome_fantasia = '{$nome_fantasia}' OR razao_social = '{$razao_social} OR cnpj = '{$cnpj}'";
// 		$resultado = mysqli_query($conexao, $query);
// 		return $resultado->num_rows;
// 	}else if($ok == 1){
// 		$query = "select * from cliente where nome_fantasia = '{$nome_fantasia}'";
// 		$resultado = mysqli_query($conexao, $query);
// 		return $resultado->num_rows;
// 	}else if($ok == 2){
// 		$query = "select * from cliente where razao_social = '{$razao_social}'";
// 		$resultado = mysqli_query($conexao, $query);
// 		return $resultado->num_rows;
// 	}else if($ok == 3){
