...
<?php
	...
	// $isCliente = searchForId2('4', $_SESSION["permissoesRotina"]);
	...
	function searchForId($id, $array) {
	   foreach ($array as $key => $val) {
	       if ($val['id_usuario'] === $id) {
	           return $key;
	       }
	   }
	   return null;
	}
	// var_dump($usuarioLogado['id_usuario']);
	...
	// $query = "select DISTINCT usuario.* from empresa INNER JOIN usuario ON empresa.id_usuario = usuario.id_usuario";
	// $resultado = mysqli_query($conexao, $query);
	// $profissionais = array();
	// while ($profissional = mysqli_fetch_assoc($resultado)) {
	// 	$key = searchForId($profissional['id_usuario'], $usuarios);
	// 	unset($usuarios[$key]);
	// 	array_push($profissionais, $profissional);
	// }

	
	// var_dump($_SESSION["permissoesRotina"]);
	$id = searchForId2('3', $_SESSION["permissoesRotina"]);
	$modo = $id;
	// $iconblock = '<img src="http://www.pvhc.net/img20/aycafkthmubmtrmrchzx.png" width="32">';
	// var_dump($_SESSION['rotina']);
	// var_dump($_SESSION["permissoesRotina"]);
...
<style>
	...
	.filtros{margin-left:15px;text-align: left;}
	.filtros h3{margin-top: 15px;margin-left: 0px;}
	.filtros h4{margin-top: 15px;margin-left: 10px;}
	.filtro{overflow-y: auto;
      display: block;
      min-height: 50px;max-height: 400px;z-index: 99;margin-left: 15px;}
    .filtros .row{margin-left: -10px!important;margin-right: -10px!important;}
	.fa-arrow-down{
		float: right;
		...
		<div class="row filtrotoggle2">
			...
			<div class="row">
				...
				<?php foreach ($usuarios as $key => $value) {
					...
					if(in_array($value['id_usuario'], $selected_usuario)){ $checked='checked';}
		...
		<div class="row filtrotoggle">
			...
			<div class="row">
				...
				<?php foreach ($filtro_trabalho as $key => $value) {
					echo "<div class='form-check row'  style='margin-left:10px;'>";
					if(in_array($value['id_trabalho'], $selected_trabalho)){ $checked='checked';}
		...
		<div class="row filtrotoggle">
			...
			<div class="row">
				...
				<?php foreach ($filtro_empresa as $key => $value) {
					echo "<div class='form-check row'  style='margin-left:10px;'>";
					if(in_array($value['id_empresa'], $selected_empresa)){ $checked='checked';}
		...
		<div class="row filtrotoggle">
			...
			<div class="row">
				...
				<?php foreach ($filtro_cliente as $key => $value) {
					echo "<div class='form-check row'  style='margin-left:10px;'>";
					if(in_array($value['id_cliente'], $selected_cliente)){ $checked='checked';}
...
<!-- 					<div class="form-group">
					...
					<!-- <div class="form-group">
						<label for="start" class="col-sm-2 control-label required">Inicio
						</label>
						<div class="col-sm-10">
							<input type="text" name="start" class="form-control date_time" id="start" required="true">
						</div>
					</div>
					<div class="form-group">
						<label for="end" class="col-sm-2 control-label required">Fim
						</label>
						<div class="col-sm-10">
							<input type="text" name="end" class="form-control date_time" id="end" required="true">
						</div>
					</div> -->
				</div>
...
// }
	...
	$('.multipledates').datepick({multiSelect: 999});
	$('.duasdatas').datepick({multiSelect: 2});
